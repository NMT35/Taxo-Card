...
// --- เสียง alias: correct = place.mp3, click = start.mp3 ---
// ขยาย playSFX ให้แปลงชื่อ alias ก่อนเล่น
const _origPlaySFX = audioManager.playSFX.bind(audioManager);
audioManager.playSFX = function (name) {
    const alias = { correct: 'place', click: 'start' };
    _origPlaySFX(alias[name] || name);
};

// เพิ่มเสียง click ให้ปุ่มทุกปุ่มอัตโนมัติ
function attachClickSounds() {
    document.querySelectorAll('button').forEach(btn => {
        if (!btn.dataset.clickSoundAttached) {
            btn.dataset.clickSoundAttached = '1';
            btn.addEventListener('click', () => audioManager.playSFX('click'), { passive: true });
        }
    });
}
...
const BOARD_SCALE_MAX = 2.0;
const BOARD_SCALE_STEP = 0.15;
...
function applyBoardScale() {
    ...
    if (label) label.textContent = Math.round(boardScale * 100) + '%';
}

function boardZoom(delta) {
    boardScale = Math.min(BOARD_SCALE_MAX, Math.max(BOARD_SCALE_MIN, boardScale + delta));
    applyBoardScale();
}
...
// Attach zoom events once DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    const viewport = document.getElementById('board-viewport');
    if (!viewport) return;

    // ── Desktop: Ctrl+Scroll or standard wheel pinch ──
    viewport.addEventListener('wheel', (e) => {
        // Only zoom when Ctrl held (standard browser zoom gesture) OR on a trackpad pinch (deltaMode 0, ctrlKey auto-set by browser)
        if (!e.ctrlKey && !e.metaKey) return;
        e.preventDefault();
        const delta = e.deltaY > 0 ? -BOARD_SCALE_STEP : BOARD_SCALE_STEP;
        boardZoom(delta);
    }, { passive: false });

    // ── Mobile/iPad: Two-finger pinch on the viewport ──
    ...
    viewport.addEventListener('touchmove', (e) => {
        if (e.touches.length === 2) {
            ...
            boardScale = Math.min(BOARD_SCALE_MAX, Math.max(BOARD_SCALE_MIN, pinchStartScale * ratio));
            applyBoardScale();
        }
    }, { passive: false });
});
...
let touchDragArmed = false;   // true after long-press delay fires
let touchLongPressTimer = null;
let touchStartX = 0;
let touchStartY = 0;
const TOUCH_DRAG_THRESHOLD = 10; // px movement to start ghost drag (after armed)
const TOUCH_LONG_PRESS_MS = 350; // ms hold before drag arms
...
function createCard(data, isEffect) {
    ...
    if (isEffect) {
        ...
        // ── Mobile: Long-press to drag, tap to view info ──
        card.addEventListener('touchstart', (e) => {
            ...
            // Start long-press timer — only arm drag after hold
            clearTimeout(touchLongPressTimer);
            touchLongPressTimer = setTimeout(() => {
                if (!game.isPlayerTurn || game.ap <= 0) return;
                touchDragArmed = true;
                // Visual + haptic feedback that drag is ready
                card.classList.add('card-long-press');
                if (navigator.vibrate) navigator.vibrate(40);
            }, TOUCH_LONG_PRESS_MS);
        }, { passive: true });

        card.addEventListener('touchmove', (e) => {
            // ถ้า 2 นิ้วขึ้นไป = pinch-to-zoom → อย่า block ให้ browser จัดการเอง
            if (e.touches.length >= 2) {
                clearTimeout(touchLongPressTimer);
                touchDragArmed = false;
                touchDragActive = false;
                removeTouchGhost();
                card.classList.remove('dragging', 'card-long-press');
                return;
            }
            ...
            // Moved too much before long-press armed → cancel the timer (treat as scroll)
            if (!touchDragArmed && dist > TOUCH_DRAG_THRESHOLD) {
                clearTimeout(touchLongPressTimer);
            }

            if (!touchDragActive && touchDragArmed && dist > TOUCH_DRAG_THRESHOLD) {
                touchDragActive = true;
                clearSelectedCard();
                game.dragData = data; game.dragEl = card;
                card.classList.remove('card-long-press');
                card.classList.add('dragging');
                touchGhost = createTouchGhost(card);
            }

            if (touchDragActive) {
                e.preventDefault(); // block scroll during active drag
                if (touchGhost) {
        ...
        card.addEventListener('touchend', (e) => {
            ...
            if (touchDragActive) {
                ...
                // Treat as TAP → open card info AND select card for tap-to-place
                touchDragArmed = false;
                const isInHand = card.closest('#player-hand') !== null;
                if (isInHand) {
                    e.preventDefault();
                    // Always open info modal so player can see skill details
                    openCardInfo(data, false, true, card);
                    // Also select the card for tap-to-place if it's player's turn
                    if (game.isPlayerTurn && game.ap > 0) {
    ...
    card.addEventListener('click', (e) => {
        // On touch devices, skip (handled by touchend above)
        if (touchDragActive) return;
        const isInHand = card.closest('#player-hand') !== null;
        // Always open card info on click — both in hand and on board
